...
files under specified web root ({0}) on each host."""
    ...
    def _perform_single(self, achall):
        ...
        for s in self.shells:
            logger.debug("Attempting to save validation to {}:{}".format(getattr(s, '_hostname', 'localhost'), validation_path))
    ...
    def cleanup(self, achalls):  # pylint: disable=missing-docstring
        for achall in achalls:
            ...
            if root_path is not None:
                validation_path = self._get_validation_path(root_path, achall)
                for s in self.shells:
                    logger.debug("Removing {}:{}".format(getattr(s, '_hostname', 'localhost'), validation_path))
                ...
                for s in self.shells:
                    try:
                        logger.debug("All challenges cleaned up, removing {}:{}".format(
                                     getattr(s, '_hostname', 'localhost'), root_path))
